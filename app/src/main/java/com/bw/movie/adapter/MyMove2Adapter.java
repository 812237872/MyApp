...
public class MyMove2Adapter extends RecyclerView.Adapter<MyMove2Adapter.ViewHolder> {
    Context context;
    List<HotMove> list;
    Listener listener;
    MovingFragment movingFragment;
    public MyMove2Adapter(Context context, List<HotMove> list, MovingFragment movingFragment) {
        ...
        viewHolder.checkBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(viewHolder.checkBox.isChecked()){
                   movingFragment.init(list.get(i).getId());
//                    list.get(i).setFollowMovie(1);
                }else{
                    movingFragment.init1(list.get(i).getId());
